...
public class LocalFlowRateRepositoryImpl implements FlowRateRepository {
    ...
    private final Map<String, List<FlowRateReadingDto>> localMap = new ConcurrentHashMap<>();

    @Override
    public Result<Optional<List<FlowRateReadingDto>>> getReadings(String zoneId, Instant start, Instant end) {
        try {
            List<FlowRateReadingDto> readings = localMap.getOrDefault(zoneId, List.of())
                    .stream()
    ...
    public Result<Void> deleteFlowRateReading(FlowRateReadingDto flowRateReading) {
        try {
            List<FlowRateReadingDto> readingsToDelete = this.localMap.get(flowRateReading.getZoneId());
